...
//Request Booking When Applying Cutoff

public class RES_SC_ID06_TEST extends ExtentTestNGReportBuilderExt {
	...
	public synchronized void testHotelConfirmationType() {
		...
		result.setAttribute("Expected", "This Booking should be in OnRequest  Status");
		try {
			...
			if (ConfirmationType.CON == ResResponse.getConfType()) {
				...
				result.setAttribute("Actual", "Test is Pass due to Confiramtion Type is : " + ResResponse.getReservationstatus());
	...
	public synchronized void isHotelOnRequestedAviResponse() {
		...
		try {
			...
			if (flag.contains("REQ")) {
				result.setAttribute("Actual", "Some rooms are On requested due to  Cutoff Restriction");
	...
	public ReservationRequest getAvailabilityData() throws Exception {
		...
		ResRequest = loader.getReservationReqObjList(ExcelDataSingleton.getInstance("Resources/Full Regression Testing Checklist.xls", "Scenario").getDataHolder())[62][0];
		return ResRequest;
