...
public class AV_SC_ID50_TEST extends ExtentTestNGReportBuilderExt{
	...
	public synchronized void availbilityTestForMinNightCondition(String TestUrl) throws Exception {
		...
		result.setAttribute("TestName", "Testing the Max night Restriction :"+ testname);
		result.setAttribute("Expected", "Results Should NOT be available due to Max night Restriction");
	...
	public AvailabilityRequest getAvailabilityData() throws Exception {
		...
		AviRequest = loader.getReservationReqObjList(ExcelDataSingleton.getInstance("Resources/Full Regression Testing Checklist.xls", "Scenario").getDataHolder())[49][0];
		return AviRequest;
