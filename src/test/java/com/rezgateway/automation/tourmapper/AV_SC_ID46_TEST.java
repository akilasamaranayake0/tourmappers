...
//Cut Off application Testing 

public class AV_SC_ID46_TEST extends ExtentTestNGReportBuilderExt {

	AvailabilityResponse AvailabilityResponse = new AvailabilityResponse();
	AvailabilityRequest AviRequest = new AvailabilityRequest();

	// Hotel id search which applies cutoff restrictions
	...
	public synchronized void isHotelOnRequested() {
		...
		result.setAttribute("Expected", "This hotel Should be On requested in given period ");
		try {
			Hotel hotelInResponse = AvailabilityResponse.getHotelList().entrySet().iterator().next().getValue();
			Map<String, ArrayList<Room>> rooms = hotelInResponse.getRoomInfo();
			ArrayList<String> flag = new ArrayList<String>();
			...
			if (flag.contains("REQ")) {
				result.setAttribute("Actual", "This hotel is onRequested Hotel due to cutoff applicaition ");
				
			} else {
				result.setAttribute("Actual", "Error - This hotel is not On Requested in the given period ");
				Assert.fail("Error - This hotel is not On Requested in the given period ");
			}
	...
	@Test(dependsOnMethods = "availbilityTest")
	public synchronized void testCutoffApplying() {

		ITestResult result = Reporter.getCurrentTestResult();
		result.setAttribute("TestName", "Testing is Cutoff applicbility For this Checkin Checkout dates ");
		result.setAttribute("Expected", "In the Response should be displyed cutoff for daily Condition. ");

		try {

			Hotel hotel = AvailabilityResponse.getHotelList().get(AviRequest.getCode()[0]);
			Map<String, ArrayList<Room>> room = hotel.getRoomInfo();
			ArrayList<String> flag = new ArrayList<String>();

			/*
			 * LocalDateTime bookingDate = LocalDateTime.now(); DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd-MMM-yyyy"); String bday = bookingDate.format(formatter); LocalDate
			 * checkingDate = LocalDate.parse(AviRequest.getCheckin(), formatter); LocalDate bookingD = LocalDate.parse(bday, formatter); long elapsedDays = ChronoUnit.DAYS.between(bookingD,
			 * checkingDate);
			 */

			for (Room r : room.entrySet().iterator().next().getValue()) {
				Map<String, RateplansInfo> ratePlanInfo = r.getRatesPlanInfo();
				TreeMap<String, DailyRates> dailyRates = ratePlanInfo.entrySet().iterator().next().getValue().getDailyRates();

				DailyRates dR = dailyRates.entrySet().iterator().next().getValue();
					if (dR.getDailyCondition().contains("CutOff")) {
						flag.add("True");
					} else {
						flag.add("False");
					}
			...
			if (flag.contains("True")) {
				result.setAttribute("Actual", "Cutoff sucessfully Applying in the given period ");
			} else {
				result.setAttribute("Actual", " Error - Cutoff is not Applying in the given period ");
				Assert.fail("  Error - Cutoff is not Applying in the given period ");
			}

		} catch (Exception e) {
			result.setAttribute("Actual", e);
			Assert.fail(" testCutoffApplicbility is fail due to :", e);
		}
	...
	public AvailabilityRequest getAvailabilityData() throws Exception {
		...
		AviRequest = loader.getReservationReqObjList(ExcelDataSingleton.getInstance("Resources/Full Regression Testing Checklist.xls", "Scenario").getDataHolder())[45][0];
		return AviRequest;
