...
public class AV_SC_ID06_TEST extends ExtentTestNGReportBuilderExt{
		...
		if (Response.getRESPONSE_CODE() == 200) {
			...
			if (AvailabilityResponse.getHotelCount() > 0) {
			    if(AvailabilityResponse.getHotelCount()==10){
					...
					result.setAttribute("Actual", "All the Hotels are not availbile in the availbile Response  : Hotel Count availbile in the Response" + AvailabilityResponse.getHotelCount());
	...
	public AvailabilityRequest getAvailabilityData() throws Exception {
		...
		AviRequest = loader.getReservationReqObjList(ExcelDataSingleton.getInstance("Resources/Full Regression Testing Checklist.xls", "Scenario").getDataHolder())[5][0];
		return AviRequest;
