...
public class AV_SC_ID16_TEST extends ExtentTestNGReportBuilderExt{
	...
	public synchronized void isHotelOnRequested(){
		...
		result.setAttribute("Expected", "this Hotel should be on OnRequest status For this Checkin Checkout dates ");
		try{
			Hotel hotelInResponse = AvailabilityResponse.getHotelList().entrySet().iterator().next().getValue();
			Map<String, ArrayList<Room>> rooms = hotelInResponse.getRoomInfo();
			ArrayList<String> flag = new ArrayList<String>();
	...
	public AvailabilityRequest getAvailabilityData() throws Exception {
		...
		AviRequest = loader.getReservationReqObjList(ExcelDataSingleton.getInstance("Resources/Full Regression Testing Checklist.xls", "Scenario").getDataHolder())[15][0];
		return AviRequest;
