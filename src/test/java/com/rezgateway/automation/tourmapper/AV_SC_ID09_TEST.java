...
public class AV_SC_ID09_TEST extends ExtentTestNGReportBuilderExt{
	...
	public synchronized void availbilityTest(String TestUrl) throws Exception {
		...
		result.setAttribute("Expected", "All the 10 hotels should availbile in the availbility Response ");
		...
		if (Response.getRESPONSE_CODE() == 200) {
			...
			if (AvailabilityResponse.getHotelCount() > 0) {
			    if(AvailabilityResponse.getHotelCount()==10){
					result.setAttribute("Actual", "Results Available for the All the Hotel IDs, Hotel Count :" + AvailabilityResponse.getHotelCount());
				}else{
					result.setAttribute("Actual", "All the Hotels are not availbile in the availbile Response  : Hotel Count availbile in the Response" + AvailabilityResponse.getHotelCount() +" / " +AvailabilityResponse.getHotelList().toString());
					Assert.fail("All the Hotels are not availbile in the availbile Response :" + AvailabilityResponse.getErrorCode());
				}
			} else {
	...
	public AvailabilityRequest getAvailabilityData() throws Exception {
		...
		AviRequest = loader.getReservationReqObjList(ExcelDataSingleton.getInstance("Resources/Full Regression Testing Checklist.xls", "Scenario").getDataHolder())[8][0];
		return AviRequest;
